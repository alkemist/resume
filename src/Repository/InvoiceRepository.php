...
class InvoiceRepository extends ServiceEntityRepository
{
     ...
     * @return Invoice[]
     */
    public function getByCompanyAndStatus(Company $company, array $status): array
    {
        $query = $this->createQueryBuilder('i');
        return $query->where('i.company = :company')
            ->setParameter('company', $company)
            ->andWhere(
                $query->expr()->in('i.status', $status)
            )
            ->getQuery()
            ->getResult();
    }
     ...
     * @throws NonUniqueResultException
     */
    public function getByCompanyLast(Company $company): Invoice|null
    {
        return $this->createQueryBuilder('i')
            ->where('i.company = :company')
