...
class InvoiceCrudController extends BaseAdminCrudController
{
    ...
    public function configureActions(Actions $actions): Actions
    {
        ...
        $payedAction = Action::new('payed', 'Marqued as payed', 'fa fa-credit-card')
            ...
            ->displayIf(fn(Invoice $invoice) => $invoice->getStatus() === InvoiceStatusEnum::Waiting
                || $invoice->getStatus() === InvoiceStatusEnum::Sent
            )
        ...
        $sendAction = Action::new('send', 'Send', 'fa fa-envelope')
            ...
            ->displayIf(
                fn(Invoice $invoice) => $invoice->getStatus() === InvoiceStatusEnum::Waiting || $invoice->getStatus(
    ...
    public function configureFields(string $pageName): iterable
    {
        if (Crud::PAGE_INDEX === $pageName) {
            ...
            yield MoneyField::new('extraHt')->setColumns(2)->setCurrency('EUR')->setStoredAsCents(
                false
            )->setNumDecimals(0);
            ...
            yield ChoiceField::new('payedBy')->setColumns(2)
                ...
                ->setFormTypeOptions([
                                         'class'   => InvoicePaymentTypeEnum::class,
                                         'choices' => InvoicePaymentTypeEnum::cases()
                                     ])
                ->setRequired(false);
            ...
            yield ChoiceField::new('status')->setColumns(2)
                ...
                ->setFormTypeOptions([
                                         ...
                                         'choices' => InvoiceStatusEnum::cases()
                                     ])
                ->setRequired(true);
    ...
    public function sendAction(AdminContext $context): Response
    {
        ...
        if ($invoice->getFilename() && count($emails) > 0) {
            $email = (new Email())
                ...
                ->replyTo($this->getParameter('MAILER_REPLY'))
                ->subject($this->getParameter('MAILER_SUBJECT') . ' Facture n°' . $invoice->getNumber())
                ->text(
                    $this->renderView(
            ...
            if ($this->getParameter('APP_ENV') === 'prod') {
                $email->to($emails[0])
                    ->addCc($this->getParameter('MAILER_CC'));
                ...
                $email
                    ->to($this->getParameter('MAILER_CC'));
            }

            try {
                $this->mailer->send($email);
            } catch (TransportExceptionInterface $e) {
                $this->flashbagService->error($e->getMessage());
            }
    ...
    {
        $oldEntity = $entityManager->getUnitOfWork()->getOriginalEntityData($entityInstance);
        $changes = $entityInstance->propertiesChanged($oldEntity);

        $isEditable = count($changes) === 1 &&
            (
                in_array('status', $changes)
                || in_array('payedAt', $changes)
                || in_array('payedBy', $changes)
                || in_array('reference', $changes)
            )
            || $oldEntity['status'] === InvoiceStatusEnum::Draft;
        ...
        if ($isEditable) {
            ...
            $entityInstance->showMessage = false;
            $this->flashbagService->send('invoice_not_editable', $entityInstance, [], 'danger');
        }
    }
}
