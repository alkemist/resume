...
class OperationFilterCrudController extends AbstractCrudController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        if (Crud::PAGE_INDEX === $pageName) {
            ...
            yield ChoiceField::new('type')->setColumns(2)
                ...
                ->setFormTypeOptions(['class' => OperationTypeEnum::class, 'choices' => OperationTypeEnum::cases()]);
            ...
            $choices = array_map(
                fn($labelAndType): string => $this->translator->trans(
                        $labelAndType['type']
                    ) . ' / ' . $labelAndType['label']
                , $lablesAndTypes
            );
    ...
    public function createEditForm(EntityDto $entityDto, KeyValueStore $formOptions, AdminContext $context
    ): FormInterface {
        $entityInstance = $entityDto->getInstance();
        $entityInstance->labelAutocomplete = $this->translator->trans(
                $entityInstance->getType()->value
            ) . ' / ' . $entityInstance->getLabel();
