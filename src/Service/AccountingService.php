...
class AccountingService
{
    ...
    public function getDashboard(?int $year, ?int $month, ?string $type): array
    {
        ...
        $totalsByYear = $this->operationRepository->getTotalsByYearAndType($type);
        $totalsByYearStats = $labels = $totalsByYearDatasets = [];
        $emptyValuesByTypes = array_fill(0, count($types), 0);
        foreach ($totalsByYear as $row) {
            if (!isset($totalsByYearStats[$row['year']])) {
                $totalsByYearStats[$row['year']] = array_combine(array_keys($types), $emptyValuesByTypes);
            }
            ...
            if ($type) {
                ...
                $totalsByYearStats[$row['year']][$row['label']] = -floatval($row['total']);
            } else {
                $totalsByYearStats[$row['year']][$row['type']->value] = -floatval($row['total']);
            }
        }

        ksort($totalsByYearStats);
        $totalsByYearLabels = array_keys($totalsByYearStats);
        $totalsByYearStats = array_values($totalsByYearStats);

        if ($type) {
            foreach ($labels as $index => $label) {
                $totalsByYearDatasets[] = [
                    'label' => $label,
                    'backgroundColor' => $colors[$index],
                    'data' => array_fill(0, count($totalsByYearLabels), 0)
                ];
            }
            foreach ($totalsByYearStats as $monthIndex => $monthValue) {
                foreach ($monthValue as $label => $total) {
                    $labelIndex = array_search($label, $labels);
                    $totalsByYearDatasets[$labelIndex]['data'][$monthIndex] = $total;
                }
            }
            $chartTotalsByYear = $this->chartBuilder->createChart(Chart::TYPE_BAR);
            $chartTotalsByYear->setData([
                'labels' => $totalsByYearLabels,
                'datasets' => $totalsByYearDatasets
            ]);
        } else {
            $chartTotalsByYear = $this->chartBuilder->createChart(Chart::TYPE_BAR);
            $chartTotalsByYear->setData([
                ...
                'datasets' => [
                    [
                        ...
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Charge->value], $totalsByYearStats),
                    ],
                    [
                        'label' => $this->translator->trans(OperationTypeEnum::Food->toString()),
                        'backgroundColor' => $colorsByTypes[OperationTypeEnum::Food->toString()],
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Food->value], $totalsByYearStats),
                    ],
                    [
                        ...
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Supply->value], $totalsByYearStats),
                    ],
                    [
                        ...
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Hobby->value], $totalsByYearStats),
                    ],
                    [
                        ...
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Subscription->value], $totalsByYearStats),
                    ],
                    [
                        ...
                        'data' => array_map(fn($item) => $item[OperationTypeEnum::Other->value], $totalsByYearStats),
                    ],
                ],
            ]);
        }

        $chartTotalsByYear->setOptions([
            'scales' => [
                'x' => [
                    'stacked' => true,
                ],
        ...
        $viewData['chartTotalsByYear'] = $chartTotalsByYear;
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////
